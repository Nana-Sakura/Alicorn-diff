...
export function initUpdator(lv?: number): void {
    BASE_URL = getString(
        "updator.url",
        `https://cdn.jsdelivr.net/gh/neila-a/LibearXL@${lv || pkg.updatorVersion + 1
        }/`,
        true
    ).replace("${version}", (lv || pkg.updatorVersion + 1).toString());
    RELEASE_FOLDER = BASE_URL + "/dist/release/";
...
async function findLatestCanUpdateVersion(): Promise<number> {
    ...
    let bu = getString(
        "updator.url",
        `https://cdn.jsdelivr.net/gh/neila-a/LibearXL@${target}/dist/release/MainBuild.json`,
        true
