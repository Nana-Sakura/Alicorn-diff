...
const CONFIG_FILE = path.resolve(getOSSpecificDataDir(), "LibearXL.config.json");

const DEFAULT_CONFIG_FILE = path.resolve(
    getBasePath(),
    "defaults",
    "LibearXL.config.json"
);
...
export async function loadConfig(): Promise<void> {
    ...
    try {
        defaultConfig = Object.freeze(
            JSON.parse(
                (
                    await fs.readFile(path.join(DEFAULTS_ROOT, "LibearXL.config.json"))
                ).toString()
            )
        );
    } catch (e) {
...
export function movOldConfigFolderSync(): void {
    const oldDir = path.resolve(os.homedir(), "LibearXL");
