...
export enum FileType {
    ...
    LibearXL_UPDATE = "LibearXLUpdate",
...
export async function justifyFileType(file: string): Promise<FileType> {
    try {
        const tmpDir = path.join(os.tmpdir(), "LibearXL-justify-file");
        ...
        if (
            ...
            for (const f of Object.values(ent)) {
                ...
                if (f.name.startsWith("LibearXL") && f.name.endsWith("Renderer.js")) {
                    return FileType.LibearXL_UPDATE;
                }
            }
