...
if (!app.requestSingleInstanceLock()) {
    console.log("Another LibearXL is running! I'm leaving now...");
    app.exit();
} else {
    READY_LOCK = true;
...
async function whenAppReady() {
    ...
    mainWindow.once("ready-to-show", async () => {
        ...
        console.log("All caught up! LibearXL is now initialized.");
        if (getBoolean("dev")) {
...
export function getMainWindowUATrimmed(): string {
    ...
    if (ua) {
        ...
        const o: string[] = [];
        uas.forEach((unit) => {
            if (!unit.includes("LibearXL") && !unit.includes("Electron")) {
                o.push(unit);
            }
        });
        return o.join(" ");
