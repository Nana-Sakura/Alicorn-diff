...
// Active DM file is ~/LibearXL/dms/active.ald

const DM_ROOT = path.join(os.homedir(), "LibearXL", "dms");
...
export function initDisplayManager(mainFile: string): void {
    ...
    DM_WINDOW.once("ready-to-show", async () => {
        ...
        console.log("All caught up! LibearXL Display Manager is now initialized.");
        if (getBoolean("dev")) {
