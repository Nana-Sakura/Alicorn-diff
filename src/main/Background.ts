...
export function registerBackgroundListeners(): void {
    ...
    ipcMain.on('window-max', function () {
        if (getMainWindow()?.isMaximized()) { // 为true表示窗口已最大化
            getMainWindow()?.restore();// 将窗口恢复为之前的状态.
        } else {
    ...
    ipcMain.on("askInject", (e) => {
        if (process.env.LibearXL_REACT_DEVTOOLS) {
            ...
            if (mw) {
                const allow = dialog.showMessageBoxSync(mw, {
                    ...
                    message:
                        "LibearXL_REACT_DEVTOOLS has been set and external scripts will be injected, which should only happen during the development.\nIf you are NOT DEVELOPNING LibearXL, this might be an XSS attack.\n\nContinue and accept external scripts to inject?",
                });
    ...
    ipcMain.handle(
        "msLogout",
        async (_e, proxy: string, key = "LibearXL_ms_login_initial") => {
