...
import { checkToGoAndDecideJump, loadToGoHook } from "./linkage/LibearXLToGo";
...
async function configureDefaultDirs(): Promise<void> {
    const pff = await alterPath(path.join(os.homedir(), "LibearXL", "pff-cache"));
    const cx = await alterPath(path.join(os.homedir(), "LibearXL", "asc-cache"));
    if (pff.length > 0) {
