...
async function startBoot(
    ...
    if (account !== null) {
        if (account.type === AccountType.MICROSOFT) {
            ...
            //}
        } else if (account.type !== AccountType.LibearXL) {
            // LibearXL don't need to flush fake token
            if (!(await account.isAccessTokenValid())) {
    ...
    if (account !== null) {
        ...
        if (
            account.type === AccountType.LibearXL &&
...
function AccountChoose(props: {
    ...
    const btnClasses = makeStyles((theme: LibearXLTheme) => ({
        btn: {
            color: theme.palette.primary.main,
                                ...
                                onClick={() => {
                                    void (async () => {
                                        ...
                                        /* window.localStorage.setItem(
                                          "MS.LoginWindowKey",
                                          "LibearXL_ms_login_" + new Date().getTime()
                                        ); */
                                        dropAccountPromise();
