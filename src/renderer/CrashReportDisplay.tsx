...
import { LibearXL_DEFAULT_THEME_LIGHT } from "./Renderer";
import { LibearXLTheme } from "./Stylex";
import { tr } from "./Translator";

const useAccStyles = makeStyles((theme: LibearXLTheme) => ({
    root: {},
    acc1: {
...
function BBCodeDisplay(props: {
    ...
    return (
        ...
        >
            ...
            <ThemeProvider theme={LibearXL_DEFAULT_THEME_LIGHT}>
                <FormControl sx={{ display: "inline", marginLeft: "auto" }}>
                    ...
                    <Button
                        onClick={() => {
                            if (
                                !copy(
                                    `Crash Report:\n[spoiler][code]${props.originCrashReport.join("\n") ||
                                    "LibearXL: Crash Report Not Found"
                                    }[/code][/spoiler]\n\nLogs:\n[spoiler][code]${props.logs
                                        .map(tab2Space)
                                        .slice(-10000) // Safe Limit
                                        .join("\n") || "LibearXL: Logs Not Found"
                                    }[/code][/spoiler]`,
                                    { format: "text/plain" }
                                )
                            ) {
